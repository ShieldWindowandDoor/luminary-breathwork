...
export default function Sleep478Breathing({ onBack }: { onBack?: () => void }) {
  ...
  // Fixed times for 4-7-8
  const inhaleTime: number = 4.0;
  const topHoldTime: number = 7.0;
  ...
  const getPhaseText = () => {
    switch(currentPhase) {
      ...
      case 'topHold': return 'Hold (7s)';
      case 'exhale': return 'Exhale (8s)';
  ...
  return (
    <div className="absolute inset-0 flex flex-col p-4 overflow-y-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            ...
            >
                <div className="max-w-md mx-auto w-full flex flex-col min-h-full pb-8 pt-4 gap-6">
                       ...
                       <div className="flex justify-between items-center mb-6">
                          ...
                          <div className="flex gap-2">
                              <button 
                                 ...
                                 className={`text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider transition-colors ${durationType === 'time' ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-300'}`}
                              >
                                 Time
                              </button>
                              <button 
                                 onClick={() => setDurationType('breaths')}
                                 className={`text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider transition-colors ${durationType === 'breaths' ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-300'}`}
                              >
                                 Breaths
                              </button>
                          </div>
                    ...
                    <div className="mt-auto pt-8 z-10 w-full mb-10">
                        <button onClick={() => setShowSettings(false)} className="w-full py-4 bg-purple-600 text-white rounded-full font-bold shadow-xl shadow-purple-500/20 hover:bg-purple-500 tracking-wider">DONE</button>
                    </div>
                </div>
            </motion.div>
         )}
      ...
      <div className="flex flex-col items-center justify-center w-full max-w-sm mx-auto my-auto min-h-max pt-20 pb-20">
         ...
         {/* Simple Circle Graphic Component for 4-7-8 */}
         <div className="relative w-48 h-48 sm:w-64 sm:h-64 mb-12 flex items-center justify-center flex-shrink-0">
            ...
            {isActive && (
                <div 
                    ...
                    style={{
                        background: currentPhase === 'inhale' ? 'radial-gradient(circle, rgba(168,85,247,0.4) 0%, rgba(168,85,247,0) 70%)' :
                                    currentPhase === 'topHold' ? 'radial-gradient(circle, rgba(168,85,247,0.7) 0%, rgba(168,85,247,0) 70%)' :
                                    currentPhase === 'exhale' ? 'radial-gradient(circle, rgba(168,85,247,0.2) 0%, rgba(168,85,247,0) 70%)' : 'transparent',
                        transform: currentPhase === 'inhale' ? `scale(${0.5 + (1 - phaseTimeLeft / inhaleTime) * 0.5})` :
                                   currentPhase === 'topHold' ? 'scale(1)' :
            ...
            <div className="relative z-10 flex flex-col items-center justify-center text-center">
               ...
               {isActive && (
                  <>
                      <span className="text-xs font-bold opacity-60 mt-2 uppercase tracking-widest text-purple-200">{phaseTimeLeft.toFixed(1)}s</span>
                  </>
               )}
            </div>
         </div>
         ...
         <div className="flex justify-center z-10 w-full mt-auto pb-8">
            {!isActive ? (
               <button 
                  ...
                  className="w-full max-w-xs px-10 py-5 rounded-full bg-purple-600 text-white font-bold hover:bg-purple-500 shadow-xl shadow-purple-500/20 tracking-wider flex items-center justify-center gap-2 transition-transform active:scale-95"
               >
