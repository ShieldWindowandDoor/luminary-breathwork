  ...
  return (
    <div className="h-full overflow-y-auto p-4 md:p-8" style={{ scrollbarWidth: "none" }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <StatCard icon={Flame} label="Streak" value={`${stats.streak}d`} />
        <StatCard icon={Sparkles} label="Level" value={`${stats.level}`} sub={`${stats.xp} XP`} />
        <StatCard icon={Wind} label="Today" value={formatDuration(stats.todaySeconds)} />
        <StatCard icon={Timer} label="Best hold" value={formatClock(stats.bestHold)} />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <button
            ...
            className="py-4 rounded-2xl bg-slate-900 border border-slate-800 font-medium hover:border-indigo-500/40"
          >
            {x.label}
          </button>
        ))}
